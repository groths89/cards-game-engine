...
def mock_dependencies():
    with patch('auth_utils.cognito_client', new=MagicMock()) as mock_client:
        yield mock_client
...
def test_require_auth_valid_token(app_with_routes):
    ...
    with app_with_routes.test_client() as client:
        with patch('auth_utils.verify_cognito_token', return_value={'success': True}):
...
def test_require_auth_invalid_token(app_with_routes):
    ...
    with app_with_routes.test_client() as client:
        with patch('auth_utils.verify_cognito_token', return_value={'success': False}):
...
def test_get_current_user_valid_token(app_with_routes):
    ...
    with app_with_routes.test_client() as client:
        with patch('auth_utils.verify_cognito_token', return_value={'success': True, 'user_id': 'test_user'}):
            response = client.get('/user', headers={'Authorization': 'Bearer mock_token'})
